...
void update_classes( std::unordered_set<TT, kitty::hash<TT>>& classes, TT tt )
{
  ...
  kitty::dynamic_truth_table ite(3);
  kitty::create_from_binary_string( ite, "00010110" );
  if(rep == ite)
  {
    printf("ITE found\n");
  }
...
void create_combinations( const std::vector<uint64_t>& start_nodes, std::map<uint64_t, uint64_t>& fos, std::vector<std::vector<uint64_t>>& adjacent_nodes )
{
  for (uint64_t i = 0; i < start_nodes.size(); ++i) {
    for (uint64_t j = i + 1; j < start_nodes.size(); ++j) {
      for(uint64_t k = j; k < start_nodes.size(); ++k){
        bool isValid = true; // Assume it's a valid pair/triplet

        // Check the secondary condition
        if(k == j) { //pair
          for (uint64_t x = i+1; x < j; ++x) // iterate over indices and not values
          {
            if (fos[start_nodes[x]] != 1) {
              isValid = false; // Not a valid pair
              break;
            }
          }
        } else { // triplet
          ...
          for (uint64_t x = j+1; x < k; ++x) // iterate over indices and not values
          {
            if (fos[start_nodes[x]] != 1) {
              isValid = false; // Not a valid triplet
              break;
            }
          }
        }

        // Check the primary condition and if it's a valid pair
        if ( k == j )
        {
          if ( isValid || k - i == 1 )
          {
            adjacent_nodes.push_back({start_nodes[i], start_nodes[j]}); // It's a valid pair
          }
        }
        else if( isValid || ( k - j == 1 && j - i == 1 ) )
        {
          adjacent_nodes.push_back({start_nodes[i], start_nodes[j], start_nodes[k]}); // It's a valid triplet
        }
      }
    }
  }
...
void erase_combinations( std::vector<std::vector<uint64_t>>& adjacent_nodes, const int cur_lvl, EnumNtk& enum_ntk )
{
...
void compute_network_structures( std::vector<EnumNtk>& Networks, std::vector<uint64_t> start_nodes, std::map<uint64_t, uint64_t> fos, EnumNtk enum_ntk, int cur_lvl, std::vector<EnumNtk::operation_t>& operations )
{
  int max_lvl = 2;
  // Code
  ++cur_lvl;
  ...
  create_combinations( start_nodes, fos, adjacent_nodes );
  // Debug output
  ...
  // Code
  erase_combinations(adjacent_nodes, cur_lvl, enum_ntk);
  // Debug output
    ...
    { // For each combination
      ...
      { // For each pair in the combination
        if ( pair.size() == 2 )
        {
          ...
          {
            auto next_to_it1 = std::next(it1);
            if(it2 > next_to_it1)
            {
              // erase from the node after `it1` till `it2` (exclusive)
              current_nodes.erase(next_to_it1, it2);
            }
            // insert `new_node` after the node at `it1` (which is not erased)
            current_nodes.insert(std::next(it1), new_node);
          }
        }
        else if ( pair.size() == 3 )
        {
          ...
          auto it1 = std::find( current_nodes.begin(), current_nodes.end(), pair[0] );
          auto it2 = std::find( current_nodes.begin(), current_nodes.end(), pair[2] );
      ...
      // Erase nodes if their FO count is 2; this might be adjusted to higher numbers
      current_nodes.erase(
          std::remove_if(current_nodes.begin(), current_nodes.end(),
                          [&current_fos](const auto& delete_node) {
                            return current_fos[delete_node] == 2;
                          }),
          current_nodes.end());
      // Debug
      ...
      compute_network_structures( Networks, current_nodes, current_fos, current_ntk, cur_lvl, operations );
    }
  }
  else
  {
    auto current_ntk = enum_ntk;
    size_t countLessThanOne = 0;
    std::vector<uint64_t> keysLessThanOne;

    for(const auto& keyValuePair : fos)
    {
      if(keyValuePair.second < 1)
      {
        ++countLessThanOne;
        keysLessThanOne.push_back(keyValuePair.first);
      }
    }

    if (countLessThanOne == 3)
    {
      EnumNtk::signal new_node = current_ntk.create_node( operations[0], keysLessThanOne[0], keysLessThanOne[1], keysLessThanOne[2] );
      fos[new_node] = 0;
      Networks.push_back( current_ntk );
      std::cout << "\nNetwork added 3" << std::endl;
    }
    else if (countLessThanOne == 2)
    {
      EnumNtk::signal new_node = current_ntk.create_node( operations[1], keysLessThanOne[0], keysLessThanOne[1] );
      fos[new_node] = 0;
      Networks.push_back( current_ntk );
      std::cout << "\nNetwork added 2" << std::endl;
    }
    else if (countLessThanOne == 1)
    {
      Networks.push_back( current_ntk );
      std::cout << "\nNetwork added 1" << std::endl;
    }
    keysLessThanOne.clear();

    std::cout << "Number of values less than 1: " << countLessThanOne << std::endl;
  }
}
                               ...
                               int numInputs, std::vector<EnumNtk::operation_t>& operations )
{
  ...
  std::map<uint64_t, uint64_t> fos;
  for ( uint64_t val : start_nodes )
  {
    fos[val] = 0;
  }
  compute_network_structures( Networks, start_nodes, fos, enum_ntk, cur_lvl, operations );
  ...
  if ( !Networks.empty() )
  {
    ...
    int n = 0;
    std::vector<int> operationIdxForNodes;
    for ( auto& network : Networks )
    {
      ...
      operationIdxForNodes.resize(binaryNodeCount);
      for ( int combination = 0; combination < combinationCount; ++combination )
      {
        ...
        { // Iterating through nodes
          if ( n.is_binary )
          {
            ...
            operationIdxForNodes[idx] = operationIdx;
            idx++;
          }
        }
        int edge_invs = 0;
        ...
        {
          // Convert the edge_invs value to a binary string
          std::string binary = std::bitset<32>(j).to_string();
          edge_invs = j;
          size_t i = 0;
...
int main()
{
  ...
  do
  {
    ...
    TT temp = std::get<0>( res );
    if ( kitty::count_zeros( temp ) >= kitty::count_ones( temp ) )
    {
      classes.insert( temp );
    }
  ...
  try_function( classes, tt_zeros );
  std::vector<TT> xs;
  for ( int i{ 0 }; i < K; ++i )
  {
    xs.emplace_back( K );
    kitty::create_nth_var( xs[i], i );
    try_function( classes, xs[i] );
  }
  ...
  //TEST
  kitty::dynamic_truth_table ttc(3);
  kitty::create_from_binary_string( ttc, "01101110" );
  if( kitty::count_ones(ttc) > kitty::count_zeros(ttc) )
  {
    ttc = ~ttc;
  }
  auto res = kitty::exact_n_canonization( ttc );
  printf("\n");
  TT temp = std::get<0>( res );
  kitty::print_binary( temp );

  kitty::create_from_binary_string( ttc, "00010110" );
  if( kitty::count_ones(ttc) > kitty::count_zeros(ttc) )
  {
    ttc = ~ttc;
  }
  res = kitty::exact_n_canonization( ttc );
  printf("\n");
  temp = std::get<0>( res );
  kitty::print_binary( temp );
